...
import {
  getSupvisorDashboard,
  type SupervisorDashboardResponse,
} from "@/api/supvisorApi";

export default function SupvisorDashboard() {
  // const [dashboardData, setDashboardData] = useState(mockDashboardData);
  const [dashboardData, setDashboardData] =
    useState<SupervisorDashboardResponse | null>(null);
  ...
  const fetchDashboard = async () => {
    try {
      setLoading(true);
      const data = await getSupvisorDashboard();
        ...
        <Card className="border-green-200 bg-gradient-to-br from-green-50 to-green-100 hover:shadow-lg transition-shadow">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-green-900 mb-2">
              {dashboardData.bloodReceivedStats.total}
            </div>
            <div className="flex items-center gap-2 text-xs">
              <span className="text-yellow-600">
                Đang tiến hành {dashboardData.bloodReceivedStats.inProgress}
              </span>
              <span className="text-red-600">
                Chưa kiểm tra {dashboardData.bloodReceivedStats.unchecked}
              </span>
              <span className="text-green-600">
                Đã kiểm tra {dashboardData.bloodReceivedStats.checked}
              </span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-orange-200 bg-gradient-to-br from-orange-50 to-orange-100 hover:shadow-lg transition-shadow">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-orange-900 mb-2">
              {dashboardData.exportRequestStats.pending}
            </div>
            <div className="flex items-center gap-2 text-xs">
              <span className="text-blue-600">
                Tổng: {dashboardData.exportRequestStats.total}
              </span>
              <span className="text-yellow-600">
                Chờ duyệt: {dashboardData.exportRequestStats.pending}
              </span>
              <span className="text-pink-600">
                Đã duyệt: {dashboardData.exportRequestStats.approved}
              </span>
              <span className="text-green-600">
                Hoàn thành: {dashboardData.exportRequestStats.finished}
              </span>
              <span className="text-red-600">
                Từ chối: {dashboardData.exportRequestStats.rejected}
              </span>
            </div>
          </CardContent>
        </Card>
