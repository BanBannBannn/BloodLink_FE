...
import { Activity, Clock, Heart, XCircle, CheckCircle } from "lucide-react";

// Import your actual API function
import { bloodDonationSummary } from "@/api/summaryApi";
...
export default function BloodDonationHistoryDashboard() {
  ...
  const fetchStatistics = async (range: number) => {
    ...
    setError(null);
    try {
      const response = await bloodDonationSummary(range);
      setStatistics(response.data);
    } catch (err) {
      setError("Không thể tải dữ liệu thống kê lịch sử hiến máu");
      console.error("Error fetching donation history statistics:", err);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">
            Thống kê lịch sử hiến máu
          </h1>
          <p className="text-gray-600 mt-1">
            Tổng quan về lịch sử các lần hiến máu
          </p>
        </div>
        ...
        <Card className="border-red-200 bg-red-50">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-900">
              {loading ? "..." : statistics?.cancelled || 0}
            </div>
            <p className="text-xs text-red-600 mt-1">Bị hủy bỏ</p>
          </CardContent>
        </Card>
